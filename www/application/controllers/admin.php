	...
	class Application_Controllers_Admin  extends Lib_BaseController {
		function index() { 
            ...
            if (isset($_GET['change_product'])) {				// изменение продукта
                ...
                if ($model->isTruby($_GET['id_class_catalog']) == true){   //если трубы
                    $this->product_info = $model->getTrubyInfo($_GET['show_id_catalog'],$_GET['id_class_catalog']);
                }
                else if ($model->isFitting($_GET['id_class_catalog']) == true){ //если фитинги
                    $this->product_info = $model->getFittingInfo($_GET['show_id_catalog'],$_GET['id_class_catalog']);
                }
                else{ //если продукты
            ...
            if (isset($_GET['button_change_data_product'])) {				// Обработать нажатие кнопки
                ...
                if ($model->isTruby($_GET['change_product_id_catalog']) == true){   //если трубы
                    echo "удалили продукт";
                }
                else if ($model->isFitting($_GET['change_product_id_catalog']) == true){ //если фитинги
                   echo "удалили фитинг";
                }
                else{ //если продукты
