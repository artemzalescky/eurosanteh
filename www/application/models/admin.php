	...
	class Application_Models_Admin {	
		...
		function getCatalogInfo($id) {	// получить инфомацию о данном каталоге (3)
			...
			do {
				...
				if ($row = mysql_fetch_object($result) || $tmp_id == 1 || $tmp_id == 14)			// если такой id уже есть (1,14 - зарезервированы за трубами и соединениями)
		...
		function deleteCatalog($id, $name) {	// удалить каталог со всеми детьми и всеми таблицами товаров (4.2)
			...
			if ($logs == null) {	// все параметры корректные
				...
				if (count($name_arr) > 1 && $name_arr[1] != 1) {	// не трубы и не соединения
				...
				if (count($name_arr) > 3  && $name_arr[1] == 1 && $name_arr[2] == 14) {	 // соединения (сами соединения не удаляем)
					$sql = "SELECT * FROM catalog WHERE name LIKE '{$name}\_%'";		// находим всех детей текущего для удаления
					$result = mysql_query($sql) or die(mysql_error());
					while ($row = mysql_fetch_object($result)) {
						$sql = "DELETE FROM catalog WHERE id='{$row->id}'";						// удаляем все подкаталоги
						mysql_query($sql) or die("Невозможно удалить каталог: ".mysql_error());	

						$sql = "SELECT * FROM fitting_class WHERE id_catalog='{$row->id}'";
						$res2 = mysql_query($sql);
						while ($row2 = mysql_fetch_object($res2)) {
							mysql_query("DELETE FROM fitting_class WHERE id='{$row2->id}'");		// удаляем записи из fitting_class
							mysql_query("DELETE FROM fitting WHERE id_class='{$row2->id}'");		// удаляем записи из fitting
						}
					}
					
					$sql = "DELETE FROM catalog WHERE id='$id'";							// удаляем запись из каталогов				
					mysql_query($sql) or die("Невозможно удалить каталог: ".mysql_error());		

					$sql = "SELECT * FROM fitting_class WHERE id_catalog='$id'";
					$res2 = mysql_query($sql);
					while ($row2 = mysql_fetch_object($res2)) {
						mysql_query("DELETE FROM fitting_class WHERE id='{$row2->id}'");		// удаляем записи из fitting_class
						mysql_query("DELETE FROM fitting WHERE id_class='{$row2->id}'");		// удаляем записи из fitting
				...
				if (count($name_arr) > 2 && $name_arr[1] == 1 && $name_arr[2] != 14) {	// трубы (сами трубы не удаляем никогда)
					$sql = "SELECT * FROM catalog WHERE name LIKE '{$name}\_%'";		// находим всех детей текущего для удаления
					$result = mysql_query($sql) or die(mysql_error());
					while ($row = mysql_fetch_object($result)) {
						$sql = "DELETE FROM catalog WHERE id='{$row->id}'";						// удаляем все подкаталоги
						...
						$sql = "SELECT * FROM truby_class WHERE id_catalog='{$row->id}'";
						$res2 = mysql_query($sql);
						while ($row2 = mysql_fetch_object($res2)) {
							mysql_query("DELETE FROM truby_class WHERE id='{$row2->id}'");		// удаляем записи из truby_class
							mysql_query("DELETE FROM truby WHERE id_class='{$row2->id}'");		// удаляем записи из truby
						}
					}
					...
					$sql = "SELECT * FROM truby_class WHERE id_catalog='$id'";
					$res2 = mysql_query($sql);
					while ($row2 = mysql_fetch_object($res2)) {
						mysql_query("DELETE FROM truby_class WHERE id='{$row2->id}'");		// удаляем записи из truby_class
						mysql_query("DELETE FROM truby WHERE id_class='{$row2->id}'");		// удаляем записи из truby
		...
		function getAllProducts($id) {
            ...
            //проверка: является ли текущий url truby_metal, truby_propilen, truby_kanaliza or fitting;
            ...
            if ($row = mysql_fetch_object($result)) {
                ...
                // если начинается с product_1 и не начинается с product_1_14
                //если трубы product_1 и нет product_1_14()
                //т.е выбрали трубы
                if (strpos($name,'product_1') === 0 && strpos($name,'product_1_14') !== 0){
                    ...
                    if ($result) {	// если такая таблица есть
                        ...
                        while ($row = mysql_fetch_object($result)) {	// если нашли
                            ...
                            $sql = "SELECT * FROM truby where id_class = $row->id";
                            $result2 = mysql_query($sql);

                            while ($row = mysql_fetch_object($result2)) {	// если нашли
                                $arr = array(
                                    "id" => $a['id'],
                                    "id_catalog" => $a["id_catalog"],
                                    "name" => $row->name,
                ...
                // если фитинги
                else if (strpos($name,'product_1_14') === 0) {
        ...
        /*
         * получение значений товара а именно труб
         * */
        function getTrubyInfo($id,$id_catalog){

            $sql = "SELECT * FROM truby_class where id_catalog = $id_catalog";
            $result = mysql_query($sql);
            ...
            if ($result) {	// если такая таблица есть
                ...
                while ($row = mysql_fetch_object($result)) {	// если нашли
                    ...
                    $sql = "SELECT * FROM truby where id = $id";
                    $result2 = mysql_query($sql);
         ...
         * получение значений товара а именно труб
         * */
        function getFittingInfo($id,$id_catalog){
            ...
            if ($result) {	// если такая таблица есть
                ...
                while ($row = mysql_fetch_object($result)) {	// если нашли
                    ...
                    $sql = "SELECT * FROM fitting where id = $id";
                    $result2 = mysql_query($sql);
        ...
        /*
     * определить по id-catalog трубы ли это если трубы возращает true
     * */
        function isTruby($id_catalog){
            $logger = null;
            ...
            if ($row = mysql_fetch_object($result)) {
                ...
                $mas = explode('_',$row->name);
                if ((count($mas) == 2) && ($mas[1] == 1)){
                    $logger = true;
                } else $logger=false;

                if ( (count($mas)>2) && ($mas[1] == 1) && ($mas[2] != 14)){
                    $logger = true;
                }
                else $logger = false;
            }
...
//                $log['name'] = 'Поле "Имя" должно быть заполнено';
	...
	function echoCatalog($id, $catalogs) {	// вывод каталога и его детей (рекурсия)
		...
		if ($id != 1) {			// ## трубы и фитинги в админку не выводим
