<?
	...
	if (count($_GET) == 1) {				// если нет параметров кроме $_GET['route']
		echo "<div style='padding: 0 0 20px 30px; color: #007;'><b>Администраторская панель</b> &nbsp&nbsp&nbsp&nbsp&nbsp<a href='/auth?logout=1' class='admin_link' style='font-size:14px'><b>Выйти</b></a></div>";
...
<form action='/admin' method='GET'>
    <table id = 'change_product_table'>
        <h2 style="text-align: center">Редактирование продукта</h2>
...
<?          if ($v == 'id'){  ?>
                <tr>
                    <td> <?=$v?> </td>
                    <td > <input type='text' name='create_product_<?=$v?>' value="<?=$last_id+1?>" onKeyDown='return false;'></td>
                </tr>
            <?} else if($v == 'id_catalog'){?>
                <tr>
                    <td> <?=$v?> </td>
                    <td > <input type='text' name='create_product_<?=$v?>'  value="<?=$_GET['current_id_catalog']?>" onKeyDown='return false;'> </td>
                </tr>

            <?} else if($v == 'description'){?>
                <tr>
                    <td> <?=$v?> </td>
                    <td > <textarea rows="4" cols="32" name='create_product_<?=$v?>'  value="<?=$_GET['current_id_catalog']?>"> </textarea> </td>
                </tr>
             <?} else{?>
                <tr>
                    <td> <?=$v?> </td>
                    <td > <input type='text' name='create_product_<?=$v?>'</td>
                </tr>
                <?}?>
        <?}?>
        <!--Вывод кнопки в таблицу-->
