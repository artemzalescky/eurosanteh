	...
	private function getRoute() {		 // получить физический путь из введенного    (.htaccess кладет в $_GET['route'] введенный маршрут)
		if (empty($_GET['route'])) {
			...
			if ($route_arr[0] == "catalog") {		// если это подраздел каталога, то обработчиком будет сам catalog, а $_GET['route'] - параметр
				...
				if (count($route_arr)>1 && strpos($route_arr[1],"truby") === 0)		// если url = catalog/tryby...  , то контроллер труб	
					$route = "truby";
					
				if (count($route_arr)>1 && strpos($route_arr[1],"fitting") === 0)		// если url = catalog/fitting...  , то контроллер фитингов	
					$route = "fitting";
			}
